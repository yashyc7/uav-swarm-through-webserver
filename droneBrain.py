...
class Drone:
    ...
    def location_callback(
        self, self2, attr_name, value
    ):  # value type is dronekit.LocationGlobalRelative
        if (
            True
        ):  # Replace tru with a way to check if any values in location object changed
            try:
                self.send_data_to_server("/dronedata", self.get_drone_data())
                alt = self.vehicle.location.global_relative_frame.alt
                # print("Alt:",alt)
                if (
                    alt < 2 and self.vehicle.mode.name == "GUIDED"
    ...
    def send_data_to_server(self, route, data):
        try:
            if not self.sitlRunning:
                if (
                    self.ip != "192.168.1.1"
                ):  # IF DRONES IP IS NOT THAT OF THE SERVER, CONTACT THE SERVER IP | <- THIS NEEDS TO BE MORE DYNAMIC
                    url = (
                        "http://" + "192.168.1.1" + ":5000" + route
                    )  # THIS WILL CAUSE AN ISSUE IF YOU TRY TO ASSIGN A PI NOT ON 1.1 TO MASTER!!!!
                else:
    ...
    def get_data_from_server(self, route, data):
        try:
            ...
            try:
                ...
                print(
                    "Failed to Parse Data To Json Object From Server, Check How The Data Is Received"
                )
                return r.text
        except:
    ...
    ):  # Sets a specified param to specified value
    ...
    def set_formation(self, formationName):
        ...
        masterLat = float(
            master_params["latitude"]
        )  # would be better to just get the location object...
        ...
        if self.formation == "triangle":
            if self.id == "1":
                ...
                self.move_to_position(
                    masterLat - 0.0000018, masterLon - 0.0000018, masterAlt
                )
    ...
    def wait_for_swarm_ready(
        self,
    ):  # THIS FUNCTION IS CURRENTLY MASTER SPECIFIC. ONLY CALL FROM MASTER
        ...
        # print("SLAVE_PARAMS CAME BACK AS: ",slave_params)
        while float(slave_params["altitude"]) <= 2 * 0.95:
    ...
    def wait_for_master_ready(
        self,
    ):  # THIS FUNCTION IS CURRENTLY SLAVE SPECIFIC. ONLY CALL FROM SLAVE
        # Will eventually need to change below to wait for each drone in the network to be ready...not just one
        master_params = self.get_data_from_server("/dronedata", {"droneID": "1"})
        ...
        # print("MASTER_PARAMS CAME BACK AS: ", master_params)
        while float(master_params["altitude"]) <= 2 * 0.95:
    ...
    def wait_for_drone_match_altitude(self, droneID):
        other_drone_params = self.get_data_from_server(
        ...
        while float(other_drone_params["altitude"]) <= 0.95 * float(
            self.get_drone_data()["altitude"]
        ):  # FIGURE OUT HOW TO DEAL IN VALUES NOT STRINGS
    ...
    def wait_for_drone_reach_altitude(self, droneID, altitude):
        ...
        print("OTHER DRONES PARAMS", other_drone_params)
        while (
            float(other_drone_params["altitude"]) <= altitude * 0.95
        ):  # FIGURE OUT HOW TO DEAL IN VALUES NOT STRINGS #ADD TRY EXCEPT FOR IF THE DATA CAME BACK AS STRING AND NOT OBJECT
            print("Drone " + droneID + " Stats: ", other_drone_params["altitude"])
            other_drone_params = self.get_data_from_server(
                "/dronedata", {"droneID": droneID}
            )
